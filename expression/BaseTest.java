...
public abstract strictfp class BaseTest {
    protected final Random random = new Random(7240958270458L);
    ...
    protected void assertEquals(final String message, final int actual, final int expected) {
    ...
    protected static String repeat(final String s, final int n) {
    ...
    private static <T> T random(final Random random, final List<T> variants) {
        return variants.get(random.nextInt(variants.size()));
    }
    ...
    protected static <T> Op<T> op(final String name, final T f) {
