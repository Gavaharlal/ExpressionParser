...
public class GenericTest extends BaseTest {
    ...
    private final List<Op<F<?>>> tests = new ArrayList<>();
    ...
    private static long check(final long v) {
        if (v != (int) v) {
    ...
    private static BigInteger bi(final int v) {
    ...
    private void all(final String expression, final F<Long> fi, final F<Double> fd, final F<BigInteger> fbi) {
        test(expression, "i", (x, y, z) -> (int) check(fi.apply(x, y, z)));
        test(expression, "d", fd);
        test(expression, "bi", fbi);
    }

    private void allConst(final String expression, final Integer v) {
        final BigInteger bi = bi(v);
        all(expression, (x, y, z) -> (long) v, (x, y, z) -> (double) v, (x, y, z) -> bi);
    }

    private void test(final String expression, final String name, final F<?> f) {
